...
export const createWaitStepHandler: StepHandlerFactory = (overridesFactory) => {
  return (ctx) => {
    const getConfig = (definition: WorkflowStepDefinition) => {
      const config = definition.waitConfig;
      invariant(config, "waitConfig must be defined for WaitStepHandler");
      return config;
    };
